...
uint64_t GetUnderLine(int64_t k, int64_t r) {
    uint64_t points_under_line = k * static_cast<uint64_t >(r) * (r + 1) / 2;
...
// y = Px/Q, x <= r
uint64_t GetOnAndUnderLine(int64_t P, int64_t Q, int64_t r, std::optional<int64_t> gcd) {
    if (gcd == std::nullopt) {
        gcd = Gcd(P, Q);
    }
    ...
    uint64_t square = (r + 1) * (std::floor(static_cast<double>(P % Q) / Q * r) + 1);
...
uint64_t GetOnAndUnderLine(int64_t P, int64_t Q, int64_t l, int64_t r, std::optional<int64_t> gcd) {
    assert (l <= r);
    if (l < 0 && r < 0) {
